...
function startRecording() {
  navigator.mediaDevices
    ...
    .then((stream) => {
      ...
      if (vb) {
        vb.innerHTML =
          '<svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><rect x="6" y="6" width="12" height="12" rx="2"/></svg>';
      }
    })
    .catch((error) => {
...
function stopRecording() {
  if (mediaRecorder && isRecording) {
    ...
    if (vb) {
      vb.innerHTML =
        '<svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M12 1.75a3.25 3.25 0 0 0-3.25 3.25v6a3.25 3.25 0 1 0 6.5 0v-6A3.25 3.25 0 0 0 12 1.75z"/><path d="M5 11.5a7 7 0 0 0 14 0"/><path d="M12 18.5v3"/></svg>';
    }
  }
}
...
function renderSuggestions(afterLi, items = []) {
  ...
  row.innerHTML = items
    .map(
      (q) =>
        `<button class="suggest-chip" type="button" data-q="${escapeHtml(
          q
        )}">${escapeHtml(q)}</button>`
    )
    .join("");
